...
export default function FooterCentrado({ tienda, acento, onScrollTo }: Props) {
  ...
  return (
    <footer id="contacto" style={{ background: 'linear-gradient(to bottom, #f1eaff, #ffffff)' }}>
      <div className="max-w-7xl mx-auto px-6 py-16 flex flex-col items-center gap-6">
        ...
        {tienda.descripcion && (
          <p className="text-center max-w-xl text-sm leading-relaxed" style={{ color: 'var(--s-muted)' }}>
            {tienda.descripcion}
          </p>
        )}
        ...
        <div className="flex gap-6 flex-wrap justify-center">
          {links.map((l) => (
            <button
              key={l.label}
              onClick={l.onClick}
              className="text-sm cursor-pointer border-none bg-transparent transition-colors hover:opacity-60"
